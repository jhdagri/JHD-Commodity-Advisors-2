...
// USDA FAS OpenData API -- X-Api-Key header (api.data.gov key)
...
exports.handler = async (event) => {
  ...
  // Back to OpenData base -- v2 path (/api/psd/) returned 404
...
function get(apiUrl, headers) {
  return new Promise(function(resolve, reject) {
    ...
    var req = https.request({ hostname: p.hostname, path: p.path, method: 'GET', headers: headers }, function(res) {
      ...
      res.on('end', function() {
        console.log('HTTP ' + res.statusCode + ' body[:300]: ' + body.slice(0, 300));
